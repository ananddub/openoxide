...
impl crate::exec::script::IntoCommand for NetworkInspectBuilder<'_> {
...
impl<'a> VolumeHandle<'a> {
    ...
    pub async fn inspect(
        &self,
        name: impl AsRef<str>,
    ) -> DockerResult<crate::docker::VolumeInspect> {
        let out = self.0.run(["volume", "inspect", name.as_ref()]).await?;
        let mut json: Vec<crate::docker::VolumeInspect> = serde_json::from_str(&out.stdout)?;
        Ok(json.pop().unwrap_or_default())
    }
