...
impl<'a> NodesHandle<'a> {
    ...
    ) -> DockerResult<crate::docker::NodeInspect> {
        let out = self.cli.run(["node", "inspect", node_id.as_ref()]).await?;
        let mut json: Vec<crate::docker::NodeInspect> = serde_json::from_str(&out.stdout)?;
    ...
    pub async fn inspect_all(
        &self,
        node_ids: &[String],
    ) -> DockerResult<Vec<crate::docker::NodeInspect>> {
