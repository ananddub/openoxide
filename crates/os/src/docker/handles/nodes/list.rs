...
impl<'a> NodeListBuilder<'a> {
    pub(crate) fn new(cli: &'a DockerCli) -> Self {
        Self {
            cli,
            args: ArgBuilder::cmd(&["node", "ls"]),
        }
    }

    pub fn filter(mut self, f: crate::docker::query::filter::NodeFilter) -> Self {
        self.args.filter(f);
        self
    }
    pub fn filters(
        mut self,
        fs: impl IntoIterator<Item = crate::docker::query::filter::NodeFilter>,
    ) -> Self {
        for f in fs {
            self.args.filter(f);
        }
    ...
    pub async fn run_json(mut self) -> DockerResult<Vec<crate::docker::NodeSummary>> {
