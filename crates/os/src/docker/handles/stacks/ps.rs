...
impl<'a> StackPsBuilder<'a> {
    pub(crate) fn new(cli: &'a DockerCli, stack_name: impl Into<String>) -> Self {
        Self {
            cli,
            args: ArgBuilder::cmd(&["stack", "ps"]),
            stack_name: stack_name.into(),
        }
    }

    pub fn filter(mut self, f: crate::docker::query::filter::TaskFilter) -> Self {
        self.args.filter(f);
        self
    }
    pub fn filters(
        mut self,
        fs: impl IntoIterator<Item = crate::docker::query::filter::TaskFilter>,
    ) -> Self {
        for f in fs {
            self.args.filter(f);
        }
    ...
    pub async fn run_json(mut self) -> DockerResult<Vec<crate::docker::TaskSummary>> {
...
impl crate::exec::script::IntoCommand for StackPsBuilder<'_> {
