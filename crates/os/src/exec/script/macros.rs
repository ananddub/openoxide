...
macro_rules! __cond_atom {
    (! dir($p:expr)) => { $crate::exec::Condition::dir_exists($p).not() };
    (! file($p:expr)) => { $crate::exec::Condition::file_exists($p).not() };
    (! cmd($c:expr)) => { $crate::exec::Condition::cmd_succeeds($c).not() };
    (! env($k:expr)) => { $crate::exec::Condition::env_set($k).not() };
    (! ($($inner:tt)+)) => { $crate::cond!($($inner)+).not() };
    (dir($p:expr)) => { $crate::exec::Condition::dir_exists($p) };
    (file($p:expr)) => { $crate::exec::Condition::file_exists($p) };
    (cmd($c:expr)) => { $crate::exec::Condition::cmd_succeeds($c) };
    (env($k:expr)) => { $crate::exec::Condition::env_set($k) };
...
macro_rules! pipeline {
    ($($tt:tt)*) => {{
        let mut __pipeline = $crate::exec::ScriptPipeline::new();
