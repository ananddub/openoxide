...
fn test_sh_macro_advanced_features() {
    ...
    let script_ir = sh!(
        ...
        cmd("echo", "deploy-finished").stdout(crate::exec::script::dsl::OutputTarget::file("/var/log/deploy.log"));
    );
...
fn test_sh_macro_convenience_dsls() {
    ...
    let script_ir = sh!(
        ...
        os.file("$tmp").exists();
        os.file("$tmp").delete();
        os.file("$tmp").chmod(crate::file::FileMode::OwnerAllGroupReadExecuteWorldReadExecute);
...
fn test_sh_macro_docker_chain_inside_if_branch() {
    use super::sh;
    let docker = crate::docker::DockerCli::new_local();
...
fn test_package_builders_and_macro_dsl() {
    ...
    let macro_script = sh!(
        os.package("nginx").install().manager(crate::package::PackageManager::Emerge);
        os.package("git").install().manager(crate::package::PackageManager::Nix);
