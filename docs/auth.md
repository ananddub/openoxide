...
```text
┌─ AUTH ────────────────────────────────────────────────────┐
│ credentials → Argon2id → JWT access/refresh → session     │
│ Optional second factor: TOTP                              │
│ Revoke: session deletion / token rotation                 │
└───────────────────────────────────────────────────────────┘
```
...
sequenceDiagram
  ...
  Auth->>Auth: Argon2id + optional TOTP
  Auth-->>Client: access token + refresh session
  Client->>Auth: refresh token
  Auth-->>Client: rotated access token
```
...
## 4. How authentication is implemented

Password verification, token creation, refresh, and session persistence are separate responsibilities. Passwords are hashed with Argon2id; JWT claims carry identity/session data, while the session repository remains the revocation source of truth. Refresh rotates access credentials instead of extending an unverified browser state.

`whoami` validates the current access token. An expired access token can produce `401`, after which the frontend calls refresh and retries. TOTP enrollment stores the secret/configuration separately and login only requires the second factor after it has been confirmed.

Authentication answers “who is this user”; organization permission middleware answers “what may this user do”. Handlers must not infer authorization from authentication claims alone.
- `id` (INTEGER PRIMARY KEY)
