...
```text
┌─ APPLICATION ─────────────────────────────────────────────┐
│ source → build → image → deploy → route → observe        │
│ State: database + deployment queue + execution history    │
│ Recovery: rollback to a recorded image/version            │
└───────────────────────────────────────────────────────────┘
```

```mermaid
flowchart LR
  API[Application API] --> S[ApplicationService]
  S --> B[Builder / source]
  B --> I[Docker image]
  I --> D[Docker deployment]
  D --> T[Traefik routing]
...
### Phase 2: Source Fetch & Build Strategy (`src/services/application/source` and `crates/os`)
...
Deployment reuses the common operation/queue path. Source resolution selects Git, Docker image, Dockerfile, Nixpacks, Railpack, or Buildpacks configuration; typed builders produce the remote commands. The resulting image is recorded before the container/Swarm workload is replaced. Traefik configuration is written only after the workload metadata is known.

Rollback does not rebuild old source. It selects a recorded deployment image/configuration, pushes/distributes the image when a registry is required, redeploys it, and records the rollback result as another auditable operation.
