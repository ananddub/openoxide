...
```text
┌─ BACKUP ──────────────────────────────────────────────────┐
│ schedule → dump/archive → encrypt/package → upload        │
│ storage: local, S3-compatible, remote rclone              │
│ restore: validate artifact → restore → record execution   │
└───────────────────────────────────────────────────────────┘
```

```mermaid
flowchart LR
  T[Schedule] --> E[Backup execution]
  E --> D[DB dump / volume archive]
  D --> U[Rclone / S3 upload]
  U --> H[Retention + history]
...
## 5. How backup execution is made recoverable

The backup definition and each execution are stored separately. A scheduled/manual trigger creates an execution row, resolves the assigned remote server and engine, writes output to a unique temporary path, validates command completion, and only then uploads or publishes the artifact.

Rclone/S3 handling is an artifact transport layer; database dump generation remains engine-specific. Volume backups archive the selected Docker volume/path. Retention runs after a successful upload so a failed new backup does not first delete the previous usable copy.

Restore resolves an existing artifact, downloads it when remote, validates the target engine/resource, and records success/failure like any other background execution.
