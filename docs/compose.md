...
```text
┌─ COMPOSE ─────────────────────────────────────────────────┐
│ source/Git → YAML parse → resource normalization           │
│         → stack deploy → Traefik route → lifecycle         │
└───────────────────────────────────────────────────────────┘
```

```mermaid
flowchart LR
  API[Compose API] --> S[ComposeService]
  S --> Y[YAML/source parser]
  Y --> N[Normalize services, env, mounts]
  N --> D[Docker stack/compose deploy]
  D --> T[Traefik routing]
```
...
On failure, stderr and operation state are persisted; cancellation targets the live operation handle. The stored Compose definition remains available for repair or redeployment.
