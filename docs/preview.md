...
```text
┌─ PREVIEW ─────────────────────────────────────────────────┐
│ PR opened/synchronized → clone/build → isolated preview    │
│ dynamic domain + routing → status tracking                 │
│ PR closed → remove preview resources and metadata          │
└───────────────────────────────────────────────────────────┘
```

```mermaid
flowchart LR
  W[PR webhook] --> P[Preview service]
  P --> C[Clone + build]
  C --> A[Ephemeral application]
  A --> D[Preview domain]
  W --> X[Closed event]
  X --> R[Cleanup]
```
...
## 4. How preview environments are solved

A preview row links the base application, generated preview application, provider/repository, pull-request number, domain, status, and last deployment. Open/synchronize events create or update this row and reuse the ordinary application deployment pipeline with PR source metadata.

The preview application is isolated through its own application ID, generated domain, and deployment history. Closing the PR cancels active work, removes the preview workload/routing, and marks the preview closed so repeated close webhooks remain safe.

Provider collaborator/signature checks occur before infrastructure is created; preview lifecycle code does not trust arbitrary branch data from an unauthenticated caller.
