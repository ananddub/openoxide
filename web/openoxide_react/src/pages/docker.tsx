...
import {
	DockerInspectModal,
	type GlobalContainerItem,
} from '#/components/docker/docker-inspect-modal';
...
function DockerPage() {
	...
	const {
		data: rawDockerContainers = [],
		isLoading: isDockerLoading,
		refetch,
		isRefetching,
	} = $api.useQuery('get', '/deployments/docker/containers', {
		params: {
			query: {
				server_id:
	...
	const {data: rawRunning, loading: isRunningLoading} =
	...
	const globalContainers: GlobalContainerItem[] = useMemo(() => {
		const dockerList = Array.isArray(rawDockerContainers)
			? rawDockerContainers
			: [];
		...
		if (dockerList.length > 0) {
			return dockerList.map((rawItem: unknown, idx: number) => {
				...
				const stateStr = String(
					item.state || item.State || item.status || '',
				).toLowerCase();
				const statusStr = String(
					item.status || item.Status || '',
				).toLowerCase();
				const isRunning =
					stateStr.includes('running') || statusStr.startsWith('up');
				...
				return {
					...
					networks: item.networks
						? String(item.networks).split(',')
						: ['bridge'],
					mounts: item.mounts
						? [
								{
									source: String(item.mounts),
									destination: '/data',
									mode: 'rw',
								},
							]
		...
		return runningList.map((rawItem: unknown, idx: number) => {
			...
			return {
				...
				image: String(
					item.docker_image || item.image || 'openoxide-app:latest',
				),
				status:
					String(item.status || item.state || 'running').toLowerCase() ===
					'running'
						? ('running' as const)
						: ('stopped' as const),
				statusText: String(
					item.status_text || item.status || 'Up (healthy)',
				),
	...
	const runningCount = globalContainers.filter(
		c => c.status === 'running',
	).length;

	return (
		<div className="mx-auto flex h-[calc(100vh-4rem)] w-full max-w-7xl animate-in flex-col gap-4 overflow-hidden p-4 duration-200 fade-in">
