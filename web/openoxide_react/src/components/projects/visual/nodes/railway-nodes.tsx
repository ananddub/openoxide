...
import {
	siPostgresql, siMysql, siRedis, siMongodb, siMariadb,
} from 'simple-icons';
...
export function AppCanvasNode({data, selected}: NodeProps<AppCanvasNodeProps>) {
...
export function DbCanvasNode({data, selected}: NodeProps<DbCanvasNodeProps>) {
	const {inspect} = useCanvasActions();
	const running = data.status === 'RUNNING' || data.status === 'running';
	...
	return (
		<div
			onClick={() => inspect({id: data.id, type: 'database', name: data.name, status: data.status})}
			onMouseEnter={() => setHov(true)}
			onMouseLeave={() => setHov(false)}
			style={{
				display: 'flex', alignItems: 'center', gap: 10,
		...
		>
			...
			<div style={{
				...
				<svg role="img" viewBox="0 0 24 24" style={{width: 14, height: 14, fill: color}}>
					<path d={icon.path} />
				</svg>
			</div>
			...
			<div style={{flex: 1, minWidth: 0}}>
				...
				<p style={{margin: 0, fontSize: 10, color: '#6e7681', marginTop: 2, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.04em'}}>
					{data.dbType}
				</p>
			</div>
