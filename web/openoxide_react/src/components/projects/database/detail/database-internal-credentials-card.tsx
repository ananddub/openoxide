...
export function DatabaseInternalCredentialsCard({database, onUpdated}: DatabaseInternalCredentialsCardProps) {
	...
	return (
		<div className="bg-card border border-border/80 rounded-2xl p-6 shadow-xs flex flex-col gap-5">
			<h3 className="text-xl font-bold text-foreground tracking-tight">Internal Credentials</h3>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 border-t border-border/40 pt-4">
					...
					<div className="flex items-center gap-2">
						{isEditingPassword ? (
							<>
								...
								/>
								<Button variant="outline" size="sm" type="button" onClick={() => setShowPassword(!showPassword)} className="shrink-0 h-9 px-2.5">
									{showPassword ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
								</Button>
								<Button variant="default" size="sm" type="button" disabled={isSavingPassword} onClick={handleSavePassword} className="shrink-0 h-9 px-3 bg-primary text-primary-foreground font-semibold text-xs gap-1.5">
									{isSavingPassword ? <RefreshCw className="size-3.5 animate-spin" /> : <Check className="size-3.5" />} Save
								</Button>
								<Button variant="ghost" size="sm" type="button" onClick={() => { setIsEditingPassword(false); setNewPassword(currentPassword); }} className="shrink-0 h-9 px-2 text-xs">
									<X className="size-3.5" />
								</Button>
							</>
						) : (
							<>
								<Input readOnly type={showPassword ? 'text' : 'password'} value={currentPassword || ''} className="font-mono text-xs bg-muted/30 h-9" />
								<Button variant="outline" size="sm" type="button" onClick={() => setShowPassword(!showPassword)} className="shrink-0 h-9 px-2.5">
									{showPassword ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
								</Button>
								<Button variant="outline" size="sm" type="button" onClick={() => setIsEditingPassword(true)} className="shrink-0 h-9 px-2.5">
				...
				<div className="flex flex-col gap-1.5 md:col-span-2">
					...
					<div className="flex items-center gap-2">
						<Input readOnly type={showInternalUrl ? 'text' : 'password'} value={internalConnStr} className="font-mono text-xs bg-muted/30 h-9" />
						<Button variant="outline" size="sm" type="button" onClick={() => setShowInternalUrl(!showInternalUrl)} className="shrink-0 h-9 px-2.5">
