...
function ServiceIcon({image}: {image: string; name?: string}) {
	...
	return (
		<div
			...
			<svg
				viewBox="0 0 24 24"
				style={{width: 20, height: 20, fill: '#1e90ff'}}>
...
export function StackServiceCard({
	...
	selected,
	id,
}: NodeProps<Node<StackServiceData>>) {
			...
			ns.map((n: any) =>
				n.id === id
					? {
							...
							data: {
								...n.data,
								replicas: Math.max(1, (n.data.replicas ?? 1) + delta),
							},
						}
					: n,
			),
	...
	const shortImage =
		data.image.length > 30 ? data.image.slice(0, 30) + '…' : data.image;
