...
export function TagSelector({
	...
	const selectedTagObjects = useMemo(() => {
		return selectedTags.map(st => {
			const found = availableTags.find(
				t => t.name.toLowerCase() === st.toLowerCase(),
			);
			return {
				name: st,
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger
				render={
					<Button
						...
						disabled={disabled}
						className={cn(
							'h-auto min-h-10 w-full cursor-pointer justify-between rounded-lg border-border/80 bg-card px-3 py-2 text-xs font-semibold shadow-2xs',
									...
									selectedTagObjects.map(t => (
										<TagBadge
											...
											<button
												...
												className="ml-1 cursor-pointer rounded-full text-muted-foreground hover:text-foreground">
												<X className="size-3" />
											</button>
										</TagBadge>
									))
			...
			<DropdownMenuContent
				className="w-[280px] rounded-xl border-border bg-popover p-2 shadow-xl"
				align="start">
				<div
					className="flex flex-col gap-2"
					onClick={e => e.stopPropagation()}>
					<div className="flex items-center justify-between gap-2 border-b border-border/40 px-1 pb-2">
						<Input
							placeholder="Search tags..."
							value={searchQuery}
							onChange={e => setSearchQuery(e.target.value)}
							className="h-8 border-border/60 bg-muted/40 text-xs"
						/>
						{selectedTags.length > 0 && (
							<Button
								variant="ghost"
							...
							filteredTags.map(tag => {
								...
								return (
									<div
										...
										className={cn(
											'flex cursor-pointer items-center justify-between rounded-lg p-2 text-xs transition-colors hover:bg-muted/60',
					...
					{searchQuery.trim() &&
						!availableTags.some(
							...
							<Button
								...
								className="mt-1 h-8 justify-start gap-1.5 rounded-none border-t border-border/40 pt-2 text-xs font-semibold text-primary">
