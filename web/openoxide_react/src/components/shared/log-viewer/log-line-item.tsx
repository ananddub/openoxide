...
export const LogLineItem = React.memo(
	({
		...
		const effectiveStream = useMemo((): typeof entry.stream => {
			...
			if (t.includes('"Error"')) {
				try {
					...
					if (start !== -1 && end !== -1) {
						const obj = JSON.parse(t.slice(start, end + 1)) as Record<
							string,
							unknown
						>;
						if (
			...
			if (
				t.startsWith('[STDERR]') ||
				/no suitable node|task failed|failed to|setup server failed|command failed|container failed|error:|panic:|fatal:|oom killed|exec format error/.test(
					lower,
				)
			) {
				return 'STDERR';
		...
		return (
			<div
				className={`group/entry relative flex w-full items-start gap-x-2 rounded-xs px-2 py-1 font-mono text-[13px] leading-relaxed transition-colors odd:bg-muted/30 even:bg-transparent hover:bg-accent/60 md:px-4 ${
				...
				{/* Line Index */}
				<span className="w-6 shrink-0 self-start pt-0.5 text-right font-mono text-[10.5px] font-normal text-muted-foreground select-none">
					{index + 1}
				</span>
				...
				{entry.containerName && (
					<ContainerTag name={entry.containerName} />
				)}
				...
				<div
					className={`min-w-0 flex-1 [word-break:break-word] whitespace-pre-wrap ${textColor}`}>
