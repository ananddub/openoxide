...
import {Activity, KeyRound, Network, RefreshCw, Save, Shield, Wrench} from 'lucide-react';
...
export function PrivateNetworkModal({open, server, onClose}: PrivateNetworkModalProps) {
	const [mode, setMode] = useState<ConnectionMode>('DIRECT_SSH');
	const [provider, setProvider] = useState<PrivateNetworkProvider>('TAILSCALE');
	...
	const managedHost = useMemo(() => remoteHostFromTunnel(tunnelAddress), [tunnelAddress]);
	...
	const input = (): UpdatePrivateNetworkInput => ({
		...
		provider: mode === 'MANAGED_WIREGUARD' ? 'WIREGUARD' : mode === 'EXTERNAL_PRIVATE_NETWORK' ? provider : null,
		private_host:
			mode === 'MANAGED_WIREGUARD' ? managedHost : mode === 'EXTERNAL_PRIVATE_NETWORK' ? privateHost.trim() : null,
		tunnel_address: mode === 'MANAGED_WIREGUARD' ? tunnelAddress.trim() : null,
		...
		persistent_keepalive: mode === 'MANAGED_WIREGUARD' ? Number(keepalive) : null,
	...
	const run = async (name: string, operation: () => Promise<unknown>, message: string) => {
		...
		try {
			...
			if (result && typeof result === 'object' && 'connection_mode' in result)
			...
			<DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						<Shield className="size-4 text-primary" /> Private Network: {server.name}
					</DialogTitle>
					<DialogDescription>Use managed WireGuard or connect through an existing private VPN.</DialogDescription>
				</DialogHeader>

				<div className="grid gap-4 py-2">
					<div className="grid gap-1.5">
						<label className="text-xs font-medium">Connection mode</label>
						<Select value={mode} onValueChange={(value) => setMode(value as ConnectionMode)} disabled={busy}>
							<SelectTrigger>
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="DIRECT_SSH">Direct SSH (no VPN)</SelectItem>
								<SelectItem value="MANAGED_WIREGUARD">Managed WireGuard</SelectItem>
								<SelectItem value="EXTERNAL_PRIVATE_NETWORK">Existing VPN / private network</SelectItem>
							</SelectContent>
						</Select>
					</div>
					...
					{mode === 'MANAGED_WIREGUARD' && (
						<>
							<div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground">
								OpenOxide installs and manages kernel WireGuard on the panel and remote server. The remote server
							...
							<div className="grid gap-3 sm:grid-cols-2">
								...
								<Field label="Remote tunnel IP">
									<Input value={managedHost || 'Invalid /24 network'} disabled />
								</Field>
								<Field label="Listen port">
									<Input type="number" value={listenPort} onChange={(event) => setListenPort(event.target.value)} />
								</Field>
								<Field label="Persistent keepalive">
									<Input type="number" value={keepalive} onChange={(event) => setKeepalive(event.target.value)} />
								</Field>
							</div>
						</>
					)}
					...
					{mode === 'EXTERNAL_PRIVATE_NETWORK' && (
						<div className="grid gap-3 sm:grid-cols-2">
							<Field label="VPN provider">
								<Select
									value={provider}
									onValueChange={(value) => setProvider(value as PrivateNetworkProvider)}
									disabled={busy}
								>
									<SelectTrigger>
										<SelectValue />
					...
					{config && (
						<div className="flex flex-wrap items-center gap-2 rounded-lg border px-3 py-2 text-xs">
							<Network className="size-3.5" /> Status: <strong>{config.status}</strong>
							<span className="text-muted-foreground">Health: {config.health_status}</span>
							{config.health_error && <span className="text-destructive">{config.health_error}</span>}
						</div>
					)}
				</div>
				...
				<DialogFooter className="flex-wrap">
					...
					{config && mode !== 'DIRECT_SSH' && (
						<>
							<Button
								...
								size="sm"
								disabled={busy}
								onClick={() => run('health', () => privateNetworkApi.health(server.id), 'Health check completed')}
							>
								<Activity /> Health
							</Button>
							<Button
								...
								size="sm"
								disabled={busy}
								onClick={() => run('repair', () => privateNetworkApi.repair(server.id), 'Private network repaired')}
							>
								<Wrench /> Repair
							</Button>
							{mode === 'MANAGED_WIREGUARD' && (
					...
					<Button onClick={() => save(mode === 'MANAGED_WIREGUARD')} disabled={busy}>
						{busy ? <RefreshCw className="animate-spin" /> : mode === 'MANAGED_WIREGUARD' ? <Shield /> : <Save />}
