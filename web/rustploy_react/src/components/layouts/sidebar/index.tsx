...
// Renders a labeled group of nav items with full-width rounded selection pill.
					...
					return (
						<SidebarMenuItem key={item.title} className="w-full">
							<SidebarMenuButton
								...
								isActive={isActive}
								tooltip={item.title}
								className={`h-8.5 w-full text-xs font-medium rounded-md transition-all px-3 flex items-center justify-start gap-2.5 ${
									isActive
							...
							{MENU.help.map(item => (
								<SidebarMenuItem key={item.title} className="w-full">
									<SidebarMenuButton
										...
										tooltip={item.title}
										className="h-8.5 w-full text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/40 rounded-md px-3 flex items-center justify-start gap-2.5 transition-all"
									>
