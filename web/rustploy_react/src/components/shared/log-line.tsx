...
// Sanitize raw Rust tracing headers e.g. "2026-07-24T21:21:30.497011Z INFO rustploy::utils::exec::exec_local: command output ... line=#12 exporting config..."
export const cleanLogLine = (line: string): string => {
	...
	// Match Rust tracing pattern: "... INFO rustploy::... line=..."
	const tracingMatch = cleaned.match(/.*(?:INFO|ERROR|WARN|DEBUG|TRACE)\s+rustploy::[^\:]+:\s*command\s+output[^\n]*?line=(.*)/i);
...
export const LogLine = React.memo(({line, index, stage}: LogLineProps) => {
	...
	// Apply cleanLogLine to strip Rust tracing headers (INFO/ERROR rustploy::... line=)
