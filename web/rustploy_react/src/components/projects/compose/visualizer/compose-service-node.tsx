...
function ServiceIcon({image}: {image: string; name?: string}) {
	...
	return (
		<div style={{
			...
			<svg viewBox="0 0 24 24" style={{width: 18, height: 18, fill: '#1e90ff'}}>
...
export function ComposeServiceNode({data, selected}: NodeProps<Node<ComposeServiceNodeData>>) {
	const envCount = Object.keys(data.envVars ?? {}).length;
	const shortImg = (data.image ?? '').length > 22
			...
			<div style={{padding: '12px 14px 10px'}}>
				...
				<div style={{display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10}}>
					...
					<div style={{flex: 1, minWidth: 0}}>
						...
						<p style={{
							margin: '2px 0 0', fontSize: 10, fontFamily: 'monospace',
							color: '#6e7681',
							whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
						}}>
							{shortImg}
				...
				{/* Config row */}
				<div style={{display: 'flex', alignItems: 'center', gap: 7, marginBottom: envCount > 0 && data.volumes.length > 0 ? 6 : 0}}>
					<SlidersHorizontal size={11} color="#6e7681" />
					<span style={{fontSize: 11, color: '#6e7681'}}>
						{envCount > 0 ? `${envCount} variable${envCount > 1 ? 's' : ''}` : 'No config required'}
					</span>
				</div>
				...
				{data.volumes.slice(0, 1).map((vol, i) => (
					<div key={i} style={{display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4}}>
						...
						<span style={{
							fontSize: 10, fontFamily: 'monospace', color: '#6e7681',
							whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
						}}>
							{vol}
						</span>
				...
				{data.ports.slice(0, 1).map((port, i) => (
					<div key={i} style={{display: 'flex', alignItems: 'center', gap: 7}}>
						...
						<span style={{fontSize: 10, fontFamily: 'monospace', color: '#6e7681'}}>
							{port}
						</span>
					</div>
				))}
