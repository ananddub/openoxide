...
import {ComposeGroupNode, ComposeServiceMiniNode, S3CanvasNode} from './nodes/compose-s3-nodes';
...
function parseServices(yaml?: string) {
	...
	try {
		...
		const list = Object.entries(svcs).map(([name, s]: [string, any]) => ({
			serviceName: name, image: (s?.image as string) || 'image',
		}));
		return list.length ? list : [{serviceName: 'web', image: 'nginx'}];
	} catch { return [{serviceName: 'web', image: 'nginx'}]; }
}
...
function buildNodes(
	applications: any[], databases: any[], composeStacks: any[],
	destinations: any[], expanded: Record<number, boolean>,
): Node[] {
	const out: Node[] = [];

	applications.forEach((app, i) => out.push({
		id: `app-${app.id}`, type: 'appNode',
		position: {x: COL_X(0), y: ROW_Y(i)},
		data: {
			id: app.id, name: app.name ?? app.app_name ?? 'App',
			status: app.status ?? 'running', framework: app.build_type ?? 'Docker',
			url: app.domain ? `https://${app.domain}` : undefined,
		},
	}));

	databases.forEach((db, i) => out.push({
		id: `db-${db.id}`, type: 'dbNode',
		position: {x: COL_X(1), y: ROW_Y(i)},
		data: {
			id: db.id, name: db.name ?? db.database_name ?? 'DB',
			dbType: db.db_type ?? 'postgres', status: db.status ?? 'running', port: db.port,
		},
	}));
	...
	composeStacks.forEach((cmp, i) => {
		...
		out.push({
			id: gid, type: 'composeGroupNode',
			position: {x: COL_X(2), y: ROW_Y(i * 2)},
			style: {
				width: isExpanded ? Math.max(count * 160 + 20, 360) : 185,
				height: isExpanded ? 140 : 'auto' as any,
			},
			data: {id: cmp.id, name: cmp.name ?? cmp.app_name ?? 'Stack',
				status: cmp.status ?? 'running', servicesCount: count, expanded: isExpanded},
		});
		children.forEach((svc, idx) => out.push({
			id: `${gid}-svc-${svc.serviceName}`, type: 'composeMiniServiceNode',
			parentId: gid, extent: 'parent',
	...
	destinations.forEach((dest, i) => out.push({
		id: `s3-${dest.id}`, type: 's3Node',
		position: {x: COL_X(3), y: ROW_Y(i)},
		data: {id: dest.id, name: dest.name ?? 'Storage',
			provider: dest.provider ?? 'S3', bucket: dest.bucket ?? ''},
	}));
...
function CanvasContent({
	applications = [], databases = [], composeStacks = [], destinations = [],
	onOpenAddApp, onOpenAddDb, onOpenAddCompose,
}: RailwayCanvasProps) {
	...
	const [arrowDrag,      setArrowDrag]      = useState<{
		ss: {x: number; y: number}; sf: {x: number; y: number}; cs: {x: number; y: number};
	} | null>(null);
	...
	useEffect(() => {
		const next = buildNodes(applications, databases, composeStacks, destinations, expandedGroups);
		const sig  = next.map(n => `${n.id}:${n.hidden}:${JSON.stringify(n.data)}`).join('|');
	...
	}, [applications, databases, composeStacks, destinations, expandedGroups]);
	...
	useEffect(() => {
		const h = (e: KeyboardEvent) => {
			...
			if (['Escape', 's', 'S', 'v', 'V'].includes(e.key)) { setMode('select'); setArrowDrag(null); }
		};
	...
	const onMouseMove = useCallback((e: React.MouseEvent) => {
		if (!arrowDrag) return;
		const rect = canvasRef.current?.getBoundingClientRect() ?? {left: 0, top: 0};
		setArrowDrag(prev => prev ? {...prev, cs: {x: e.clientX - rect.left, y: e.clientY - rect.top}} : null);
	}, [arrowDrag]);

	const onMouseUp = useCallback((e: React.MouseEvent) => {
		if (!arrowDrag) return;
		const endFlow = screenToFlowPosition({x: e.clientX, y: e.clientY});
		const dx = endFlow.x - arrowDrag.sf.x, dy = endFlow.y - arrowDrag.sf.y;
		if (Math.sqrt(dx * dx + dy * dy) < 10) { setArrowDrag(null); return; }
		const ts = Date.now();
		const sid = `anc-${ts}-s`, eid = `anc-${ts}-e`;
		setNodes(prev => [...prev,
			{id: sid, type: 'arrowAnchor' as any, position: arrowDrag.sf, data: {}},
			{id: eid, type: 'arrowAnchor' as any, position: endFlow, data: {}},
		]);
		setEdges(prev => [...prev, {
			id: `arrow-${ts}`, source: sid, target: eid,
			type: 'customEdge', animated: false, data: {},
		}]);
		setArrowDrag(null);
	...
	const onConnect = useCallback((conn: Connection) => {
		...
		if (srcApp && tgtDb) setPendingConn({
			sourceId: srcId, targetId: tgtId,
			sourceName: srcApp.name ?? srcApp.app_name,
			targetName: tgtDb.name ?? tgtDb.database_name, targetDbType: tgtDb.db_type,
		});
		setEdges(eds => addEdge({...conn, type: 'customEdge', data: {}}, eds));
	...
	const onPaneContextMenu = useCallback((e: React.MouseEvent | MouseEvent) => {
		...
		const b = (e.currentTarget as HTMLElement)?.getBoundingClientRect() ?? {left: 0, top: 0};
		setCtxMenu({x: x - b.left, y: y - b.top});
	}, [mode]);

	const ctxActions = useMemo(() => ({
		inspect: setActiveNode,
		toggleExpand: (id: number) => setExpandedGroups(p => ({...p, [id]: !p[id]})),
	}), []);
	...
	return (
		<CanvasActionsContext.Provider value={ctxActions}>
			<div
				...
				style={{
					height: 640, borderRadius: 12, overflow: 'hidden',
					background: '#0d1117',
			...
			>
				<ReactFlow
					...
					connectionLineStyle={{stroke: '#58a6ff', strokeWidth: 1.5, strokeDasharray: '5 4', opacity: 0.7}}
				...
				{mode === 'arrow' && arrowDrag && (
					<svg className="absolute inset-0 pointer-events-none z-20"
						...
						<defs>
							<marker id="prev-mk" markerWidth="8" markerHeight="8"
								refX="6" refY="4" orient="auto" markerUnits="userSpaceOnUse">
								<path d="M0,0.5 L6.5,4 L0,7.5 L1.5,4 Z" fill="#58a6ff" opacity="0.85" />
							</marker>
						</defs>
						<circle cx={arrowDrag.ss.x} cy={arrowDrag.ss.y} r={3.5} fill="#58a6ff" opacity="0.8" />
						<line
							x1={arrowDrag.ss.x} y1={arrowDrag.ss.y}
							x2={arrowDrag.cs.x} y2={arrowDrag.cs.y}
				...
				<div style={{
					...
					padding: '3px 5px', borderRadius: 99,
					background: '#161b22',
					border: '1px solid rgba(255,255,255,0.08)',
					boxShadow: '0 1px 8px rgba(0,0,0,0.3)',
				}}>
					{[
						{icon: <MousePointer2 size={13} />, label: 'Select  V', key: 'select' as const},
						{icon: <MoveRight size={13} />, label: 'Arrow  A', key: 'arrow' as const},
					].map(t => (
						<button
							key={t.key}
							type="button"
							title={t.label}
							onClick={() => { setMode(t.key); setArrowDrag(null); }}
							style={{
								display: 'flex', alignItems: 'center', gap: 5,
								padding: '4px 10px', borderRadius: 99, border: 'none', cursor: 'pointer',
								fontSize: 11.5, fontWeight: 500,
								background: mode === t.key
									? (t.key === 'arrow' ? 'rgba(88,166,255,0.15)' : 'rgba(255,255,255,0.08)')
									: 'transparent',
								color: mode === t.key
									? (t.key === 'arrow' ? '#58a6ff' : '#e6edf3')
									: '#6e7681',
								transition: 'all .12s',
							}}
						>
							{t.icon}
							{t.key === 'arrow' && mode === 'arrow' ? 'Arrow' : t.key === 'select' && mode === 'select' ? 'Select' : ''}
						</button>
					))}
				</div>
				...
				{mode === 'arrow' && (
					<div style={{
						...
						transform: 'translateX(-50%)', zIndex: 20,
						background: '#161b22', border: '1px solid rgba(88,166,255,0.3)',
						borderRadius: 99, padding: '4px 14px',
						fontSize: 11, color: '#58a6ff', fontWeight: 500,
				...
				{ctxMenu && (
					<div
						...
						style={{
							position: 'absolute', top: ctxMenu.y, left: ctxMenu.x, zIndex: 50,
							background: '#161b22', border: '1px solid rgba(255,255,255,0.1)',
							borderRadius: 10, padding: 4, minWidth: 160,
							boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
						}}
					>
						<p style={{margin: '2px 10px 6px', fontSize: 10, fontWeight: 600,
				...
				{pendingConn && (
					<ServiceLinkerModal
						isOpen onClose={() => setPendingConn(null)}
						sourceNode={{id: pendingConn.sourceId, name: pendingConn.sourceName, type: 'app'}}
						targetNode={{id: pendingConn.targetId, name: pendingConn.targetName, type: 'database', dbType: pendingConn.targetDbType}}
						onConfirmLink={async () => setPendingConn(null)}
					/>
				)}

				<ServiceInspectorDrawer
					isOpen={!!activeNode} onClose={() => setActiveNode(null)}
