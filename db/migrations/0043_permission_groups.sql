-- Organization-scoped group assignments. Existing users keep their current group.
...
SET group_id = (SELECT group_id FROM users WHERE users.id = organization_members.user_id)
