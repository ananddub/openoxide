...
use crate::{
    api::dto::application::patch::{ApplicationPatchResponseDto, UpsertApplicationPatchDto},
    core::middleware::{permission::RequirePermission, validator::ValidatedJson},
    services::application::patch::PatchService,
};

type ApiError = (StatusCode, String);

pub struct PatchController {
    service: Arc<PatchService>,
}

#[controller("/applications/{application_id}/patches")]
impl PatchController {
    fn new(service: Arc<PatchService>) -> Self {
        Self { service }
    }
    ...
    ) -> Result<Json<Vec<ApplicationPatchResponseDto>>, ApiError> {
        self.service
            .list(application_id)
            .await
    ...
    async fn create(
        ...
        Path(application_id): Path<i64>,
        ValidatedJson(body): ValidatedJson<UpsertApplicationPatchDto>,
    ) -> Result<(StatusCode, Json<ApplicationPatchResponseDto>), ApiError> {
        self.service
    ...
    async fn update(
        ...
        ValidatedJson(body): ValidatedJson<UpsertApplicationPatchDto>,
    ) -> Result<Json<ApplicationPatchResponseDto>, ApiError> {
        self.service
    ...
    async fn delete(
        ...
        match self.service.delete(application_id, id).await {
            ...
            Ok(false) => Err((StatusCode::NOT_FOUND, "patch not found".into())),
...
fn map_error(error: sqlx::Error) -> ApiError {
    match error {
        sqlx::Error::RowNotFound => (
            ...
            "application or patch not found".into(),
        ),
        sqlx::Error::Protocol(message) => (StatusCode::BAD_REQUEST, message),
        sqlx::Error::Database(ref error) if error.is_unique_violation() => (
            StatusCode::CONFLICT,
            "a patch already exists for this file".into(),
        ),
        other => {
            tracing::error!(error = %other, "application patch operation failed");
            (
