...
for cycle in 1 2 3 4 5; do
    ...
    nsenter -t "$peer_a" -n ip link del wg-a
    nsenter -t "$peer_b" -n ip link del wg-b
    ! nsenter -t "$peer_a" -n ip link show wg-a
    ! nsenter -t "$peer_b" -n ip link show wg-b
    ! nsenter -t "$peer_a" -n ip route show 10.90.0.2/32 | grep -q .
    ! nsenter -t "$peer_b" -n ip route show 10.90.0.1/32 | grep -q .
done
"#;
