...
impl VaultProviderRepository {
    ...
    pub async fn list_by_organization(&self, organization_id: i64) -> Result<Vec<VaultProvider>, sqlx::Error> {
        sqlx::query_as!(
            VaultProvider,
    ...
    pub async fn update(&self, id: i64, item: &VaultProvider) -> Result<(), sqlx::Error> {
        sqlx::query!(
            r#"UPDATE vault_providers SET name = ?, provider_type = ?, api_url = ?, auth_token = ?, namespace = ?, config_json = ?, updated_at = ? WHERE id = ?"#,
            &item.name,
            &item.provider_type,
            &item.api_url,
            &item.auth_token,
