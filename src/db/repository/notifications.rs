...
impl NotificationRepository {
    ...
    pub async fn create(&self, item: &Notification) -> Result<i64, sqlx::Error> {
        let _res = sqlx::query!(
            r#"INSERT INTO notifications (name, notification_type, on_app_deploy, on_app_build_error, on_database_backup, on_volume_backup, on_panel_restart, on_docker_cleanup, on_server_threshold, on_panel_backup, on_schedule_success, on_schedule_failure, slack_id, telegram_id, discord_id, email_id, resend_id, gotify_id, ntfy_id, mattermost_id, custom_id, lark_id, pushover_id, teams_id, organization_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#,
            &item.name,
            ...
            item.on_panel_backup,
            item.on_schedule_success,
            item.on_schedule_failure,
            item.slack_id,
            item.telegram_id,
            item.discord_id,
            item.email_id,
    ...
    pub async fn update(&self, id: i64, item: &Notification) -> Result<(), sqlx::Error> {
        sqlx::query!(
            r#"UPDATE notifications SET name = ?, notification_type = ?, on_app_deploy = ?, on_app_build_error = ?, on_database_backup = ?, on_volume_backup = ?, on_panel_restart = ?, on_docker_cleanup = ?, on_server_threshold = ?, on_panel_backup = ?, on_schedule_success = ?, on_schedule_failure = ?, slack_id = ?, telegram_id = ?, discord_id = ?, email_id = ?, resend_id = ?, gotify_id = ?, ntfy_id = ?, mattermost_id = ?, custom_id = ?, lark_id = ?, pushover_id = ?, teams_id = ?, organization_id = ?, created_at = ?, updated_at = ? WHERE id = ?"#,
            &item.name,
            &item.notification_type,
            item.on_app_deploy,
            item.on_app_build_error,
