...
impl ServerMetricRepository {
    ...
    pub async fn get_all(&self) -> Result<Vec<ServerMetric>, sqlx::Error> {
        sqlx::query_as!(
            ServerMetric,
            r#"SELECT timestamp AS "timestamp?: i64", server_id AS "server_id: i64", cpu AS "cpu: f64", cpu_model AS "cpu_model: String", cpu_cores AS "cpu_cores: i64", cpu_physical_cores AS "cpu_physical_cores: i64", cpu_speed AS "cpu_speed: f64", os AS "os: String", distro AS "distro: String", kernel AS "kernel: String", arch AS "arch: String", mem_used AS "mem_used: f64", mem_used_gb AS "mem_used_gb: f64", mem_total AS "mem_total: f64", uptime AS "uptime: i64", disk_used AS "disk_used: f64", total_disk AS "total_disk: f64", network_in AS "network_in: f64", network_out AS "network_out: f64" FROM server_metrics"#
        )
    ...
    pub async fn get_latest_per_server(&self) -> Result<Vec<ServerMetric>, sqlx::Error> {
        sqlx::query_as!(
            ServerMetric,
            r#"SELECT timestamp AS "timestamp?: i64", server_id AS "server_id: i64", cpu AS "cpu: f64", cpu_model AS "cpu_model: String", cpu_cores AS "cpu_cores: i64", cpu_physical_cores AS "cpu_physical_cores: i64", cpu_speed AS "cpu_speed: f64", os AS "os: String", distro AS "distro: String", kernel AS "kernel: String", arch AS "arch: String", mem_used AS "mem_used: f64", mem_used_gb AS "mem_used_gb: f64", mem_total AS "mem_total: f64", uptime AS "uptime: i64", disk_used AS "disk_used: f64", total_disk AS "total_disk: f64", network_in AS "network_in: f64", network_out AS "network_out: f64"
               FROM server_metrics sm
               WHERE timestamp = (SELECT MAX(timestamp) FROM server_metrics WHERE server_id = sm.server_id)"#
        )
        .fetch_all(self.pool.as_ref())
        .await
    }
    ...
    pub async fn create(&self, item: &ServerMetric) -> Result<i64, sqlx::Error> {
        let _res = sqlx::query!(
            r#"INSERT INTO server_metrics (server_id, cpu, cpu_model, cpu_cores, cpu_physical_cores, cpu_speed, os, distro, kernel, arch, mem_used, mem_used_gb, mem_total, uptime, disk_used, total_disk, network_in, network_out) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#,
            item.server_id,
            item.cpu,
            &item.cpu_model,
            item.cpu_cores,
    ...
    pub async fn delete_older_than(&self, cutoff: i64) -> Result<u64, sqlx::Error> {
        Ok(sqlx::query("DELETE FROM server_metrics WHERE timestamp < ?")
