...
impl BitbucketProviderRepository {
    ...
    pub async fn get_by_git_provider_id(
        &self,
        id: i64,
    ) -> Result<Option<BitbucketProvider>, sqlx::Error> {
        sqlx::query_as!(BitbucketProvider, r#"SELECT id AS "id?: i64", bitbucket_username AS "bitbucket_username?: String", bitbucket_email AS "bitbucket_email?: String", app_password AS "app_password?: String", api_token AS "api_token?: String", bitbucket_workspace_name AS "bitbucket_workspace_name?: String", git_provider_id AS "git_provider_id: i64", created_at AS "created_at: i64", updated_at AS "updated_at: i64" FROM bitbucket_providers WHERE git_provider_id = ?"#, id).fetch_optional(self.pool.as_ref()).await
    }
    ...
    pub async fn delete(&self, id: i64) -> Result<(), sqlx::Error> {
        sqlx::query!(r#"DELETE FROM bitbucket_providers WHERE id = ?"#, id)
            .execute(self.pool.as_ref())
            .await?;
        Ok(())
    }
    ...
    pub async fn disconnect(&self, git_provider_id: i64) -> Result<bool, sqlx::Error> {
        let result = sqlx::query!(
            "UPDATE bitbucket_providers SET app_password = NULL, api_token = NULL, updated_at = strftime('%s', 'now') WHERE git_provider_id = ?",
            git_provider_id
        )
        .execute(self.pool.as_ref())
        .await?;
