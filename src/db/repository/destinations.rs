...
impl DestinationRepository {
    ...
    pub async fn create(&self, item: &Destination) -> Result<i64, sqlx::Error> {
        let _res = sqlx::query!(
            r#"INSERT INTO destinations (name, provider, access_key, secret_access_key, bucket, region, endpoint, organization_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#,
            &item.name,
            &item.provider,
            &item.access_key,
    ...
    pub async fn update(&self, id: i64, item: &Destination) -> Result<(), sqlx::Error> {
        sqlx::query!(
            r#"UPDATE destinations SET name = ?, provider = ?, access_key = ?, secret_access_key = ?, bucket = ?, region = ?, endpoint = ?, organization_id = ?, created_at = ?, updated_at = ? WHERE id = ?"#,
            &item.name,
            &item.provider,
            &item.access_key,
            &item.secret_access_key,
    ...
    pub async fn delete(&self, id: i64) -> Result<(), sqlx::Error> {
        sqlx::query!(r#"DELETE FROM destinations WHERE id = ?"#, id)
            .execute(self.pool.as_ref())
            .await?;
        Ok(())
    }
